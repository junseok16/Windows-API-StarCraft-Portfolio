...
void CGhstProjectile::Initialize()
{
	...
	************************/
	...
	m_pProperty = CPropertyManager::GetManager()->GetUnitProperty(ETerranUnitType::GHOST);
}

void CGhstProjectile::Update() {
	CProjectile::Update();
...
void CGhstProjectile::LateUpdate() {
	CProjectile::LateUpdate();
}

void CGhstProjectile::Render(HDC _hDC) {
	CProjectile::Render(_hDC);
}

void CGhstProjectile::Release() {
	CProjectile::Release();
}
